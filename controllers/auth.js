...
exports.signWithGoogle = catchAsync(async (req, res, next) => {
  ...
  if (!req.body) return next(new AppError('Please provide profile object', 400));
  let user = await User.findOne({ email });
  if (!user) {
    // User does not exist, create new user with dummy password values
    user = await User.create({
      email,
      fullName,
      photo,
      password: 'google-password',
      passwordConfirm: 'google-password',
    });
    user.new = true;
  } else user.new = false;
